...
namespace motor_control {
    ...
    Feedback_t MotorUtilities::read() {
        UINT8_T error = 0;
        int comm = 0;
        static std::uint64_t cyclecounter = 0;
        static Feedback_t feedback;

        union four_byte_data_t {
            std::uint8_t raw[4];
            std::int32_t int32_value;
        };

        union two_byte_data_t {
            std::uint8_t raw[2];
            std::int16_t int16_value;
        };

        feedback.velocities.clear();
        feedback.positions.clear();
        feedback.torques.clear();

        four_byte_data_t temp1;
        std::int16_t temp2;

        std::uint8_t raw_values[10];
        for (auto const motor : motors_) {
            std::int32_t raw_position=0, raw_velocity=0, raw_torque=0;
            comm = -1;
            int counter =0;
            while(comm != 0) {
                counter++;
                comm = packet_handler_->ReadTxRx(port_handler_, motor.id, motor.tool->ctrl_table_["present_position"]->address, 8 , raw_values, &error);
                comm = packet_handler_->Read2ByteTxRx(port_handler_, motor.id, motor.tool->ctrl_table_["present_current"]->address, (uint16_t*)&temp2, &error);
                if(comm != 0) {
                    std::cout << "Error reading data for motor " << static_cast<int>(motor.id) << " (" << motor.tool->model_name_ << ")" << std::endl;
                }
                if(counter > 10) {
            ...
            // map raw data read from motors to useable integers
            temp1.raw[0] = raw_values[0];
            temp1.raw[1] = raw_values[1];
            temp1.raw[2] = raw_values[2];
            temp1.raw[3] = raw_values[3];

            raw_position = temp1.int32_value;

            temp1.raw[0] = raw_values[4];
            temp1.raw[1] = raw_values[5];
            temp1.raw[2] = raw_values[6];
            temp1.raw[3] = raw_values[7];

            raw_velocity = temp1.int32_value;

            raw_torque = temp2;

            double rad_per_tick = motor.tool->max_radian_ / motor.tool->value_of_max_radian_position_;
            feedback.positions.push_back(double(raw_position*rad_per_tick));
            feedback.velocities.push_back(double(raw_velocity*rad_per_tick));
            feedback.torques.push_back(double(raw_torque*CURRENT_TO_TORQUE_RATIO_));

            if((cyclecounter % 50L) == 0)
            {
                std::cout << "motor " << static_cast<int>(motor.id) << std::endl;
                std::cout << "position " << feedback.positions[static_cast<int>(motor.id)-1] << std::endl;
                std::cout << "velocity " << feedback.velocities[static_cast<int>(motor.id)-1] << std::endl;
                std::cout << "torque "   << feedback.torques[static_cast<int>(motor.id)-1] << std::endl;
                std::cout << "torque raw " << raw_torque  << std::endl << std::endl;
            }
        }
        if((cyclecounter % 50L) == 0)
        {
            std::cout << "cyclecounter: " << cyclecounter << std::endl << std::endl;
        }
        cyclecounter++;
        return feedback;
    }
